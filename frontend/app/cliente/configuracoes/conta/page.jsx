      ...
      <section className="mx-auto w-full max-w-4xl flex-1 px-5 py-10 sm:py-14">
        ...
        <form onSubmit={enviarFormulario} className="mt-10 rounded-[8px] bg-white p-6 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:grid sm:grid-cols-2 sm:gap-6 sm:p-8">
          <label className="grid gap-2 sm:col-span-2">
            <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-app-cinza">{ui("Nome completo")}</span>
          ...
          <label className="mt-6 grid gap-2 sm:mt-0">
            <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-app-cinza">{ui("Data de nascimento")}</span>
          ...
          <label className="mt-6 grid gap-2 sm:mt-0">
            <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-app-cinza">{ui("Endereço de e-mail")}</span>
          ...
          <label className="mt-6 grid gap-2 sm:mt-0">
            <span className="text-[10px] font-bold uppercase tracking-[0.22em] text-app-cinza">{ui("Telefone")}</span>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:col-span-2 sm:flex-row">
            ...
            <Link href="/cliente/configuracoes" className="flex h-11 items-center justify-center rounded-[8px] border border-app-mocha px-8 text-xs font-bold uppercase tracking-wide text-app-mocha transition hover:bg-app-chantilly">{ui("Cancelar")}</Link>
          </div>
        </form>
        ...
        <aside className="mt-8 rounded-[8px] bg-white p-5 ring-1 ring-app-baunilha-dourada/60 sm:p-6">
          <div className="flex gap-3">
            ...
            <div>
              <h3 className="text-xs font-bold uppercase tracking-[0.18em] text-app-mocha">{ui("Privacidade de dados")}</h3>
              <p className="mt-2 text-sm leading-5 text-app-cinza">{ui("Suas informações são utilizadas para identificação da conta e segurança de acesso.")}</p>
            </div>
          </div>
        </aside>
