...
const reservation: ReservationDetails | null = null;
...
function Icon({
  ...
  const paths = {
    ...
    receipt:
      "M6 3h12v18l-2-1.2-2 1.2-2-1.2-2 1.2-2-1.2L6 21V3z M9 8h6M9 12h6M9 16h4",
    shield:
      "M12 21s7-3.2 7-9.8V5l-7-3-7 3v6.2C5 17.8 12 21 12 21z M9.5 12l1.7 1.7 3.8-4",
    users:
...
function DetailSlot({
  ...
  return (
    <div className="flex gap-4">
      <Icon
        type={icon}
        className="mt-7 h-5 w-5 shrink-0 text-app-caramelo-torrado"
      />
      <div>
        ...
        <div className="mt-12 flex flex-col gap-8 border-b border-app-baunilha-dourada pb-8 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-8 flex items-center gap-2 text-lg text-app-cinza">
              <Icon
                type="shield"
                className="h-5 w-5 text-app-caramelo-torrado"
              />
          ...
          <div className="flex flex-col gap-3 sm:flex-row lg:pb-2">
            <button
              ...
              className="h-12 rounded-[8px] bg-app-caramelo-torrado px-10 text-xs font-bold uppercase text-app-chantilly transition hover:bg-app-cafe-profundo disabled:cursor-not-allowed disabled:bg-app-cinza/35 disabled:text-app-chantilly"
            >
              Ver recibo
            </button>
            <button
              type="button"
              disabled={!hasReservation}
              className="h-12 rounded-[8px] border border-app-mocha px-10 text-xs font-bold uppercase text-app-mocha transition hover:bg-app-creme-leve disabled:cursor-not-allowed disabled:border-app-cinza/40 disabled:text-app-cinza"
            >
              Entrar em contato
            </button>
          </div>
            ...
            {reservation?.items.length ? (
              <div className="mt-8 grid gap-6">
                {reservation.items.map((item) => (
                  <article
                    key={item.id}
                    className="grid gap-4 border-b border-app-baunilha-dourada/60 pb-6 md:grid-cols-[1fr_auto]"
                  >
                    <div>
        ...
        <aside className="grid h-fit gap-4">
          <article className="overflow-hidden rounded-[8px] bg-app-creme-leve shadow-sm ring-1 ring-app-baunilha-dourada/60">
            ...
            <div className="p-6 sm:p-8">
              ...
              <button
                type="button"
                disabled={!reservation?.restaurant?.mapsUrl}
                className="mt-8 h-12 w-full rounded-[8px] border border-app-mocha text-xs font-bold uppercase text-app-mocha transition hover:bg-app-chantilly disabled:cursor-not-allowed disabled:border-app-cinza/40 disabled:text-app-cinza"
              >
          ...
          <article className="rounded-[8px] bg-app-creme-leve p-8 text-center shadow-sm ring-1 ring-app-baunilha-dourada/60">
            <Icon
              type="help"
              className="mx-auto h-6 w-6 text-app-caramelo-torrado"
            />
            <p className="mt-4 text-app-cinza">Dúvidas sobre sua reserva?</p>
            <Link
              href="#"
              className="mt-3 inline-block font-bold text-app-cafe-profundo underline underline-offset-4"
            >
          ...
          <button
            type="button"
            disabled={!hasReservation}
            className="mx-auto mt-4 flex items-center gap-3 text-xs font-bold uppercase text-app-vermelho-erro transition hover:text-app-cafe-profundo disabled:cursor-not-allowed disabled:text-app-cinza"
          >
