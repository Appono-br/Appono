...
export default function FavoritosPage() {
    ...
    useEffect(() => {
        ...
        apiRequest("/restaurantes", { signal: controller.signal }).then((data) => { setRestaurantes(data ?? []); setMensagem(""); }).catch((error) => {
            if (error?.name !== "AbortError") setMensagem(error instanceof Error ? error.message : "Nao foi possivel carregar favoritos.");
        });
    ...
    async function remover(restaurante) {
        ...
        try {
            await apiRequest(`/restaurantes/${restaurante.id_restaurante}/favorito`, { method: "PATCH", body: JSON.stringify({ favorito: false }) });
