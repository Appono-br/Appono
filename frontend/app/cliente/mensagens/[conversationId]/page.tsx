...
function Icon({
  ...
  type: "arrow-left" | "info" | "message" | "paperclip" | "phone" | "send" | "smile";
  className?: string;
}) {
  const paths = {
    ...
    <main className="flex min-h-screen flex-col bg-app-chantilly text-app-cafe-profundo">
      ...
      <section className="mx-auto flex w-full max-w-7xl flex-1 flex-col px-5 py-10">
        ...
        {messages.length ? null : (
          <form className="mt-8 flex items-center gap-3 rounded-[8px] bg-app-creme-leve p-4 shadow-sm ring-1 ring-app-baunilha-dourada/60">
            <button
              type="button"
              disabled
              className="text-app-cinza"
            ...
            <button
              type="submit"
              disabled
              className="flex h-12 w-12 items-center justify-center rounded-[8px] bg-app-dourado-mel/45 text-white"
