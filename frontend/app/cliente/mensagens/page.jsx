      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        <div className="overflow-hidden rounded-[24px] bg-app-cafe-profundo text-app-creme-leve shadow-sm">
          <div className="grid gap-8 p-6 sm:p-8 lg:grid-cols-[1fr_390px] lg:items-end">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-app-baunilha-dourada">Conversas seguras</p>
              <h1 className="mt-3 text-4xl font-semibold leading-tight sm:text-6xl">Mensagens</h1>
              <p className="mt-4 max-w-2xl text-sm leading-6 text-app-creme-suave">Centralize conversas com restaurantes antes da visita, com contexto de reserva ou pedido quando existir.</p>
            </div>
            <div className="grid gap-3 rounded-[18px] bg-white/10 p-3 ring-1 ring-white/10">
              <label className="flex h-12 items-center gap-3 rounded-[12px] bg-white px-4 text-app-cafe-profundo shadow-sm transition focus-within:ring-2 focus-within:ring-app-dourado-mel/35">
                ...
                <input value={busca} onChange={(event) => setBusca(event.target.value)} placeholder="Buscar restaurante ou mensagem" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-app-cinza/60" />
              </label>
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-[12px] bg-white/10 px-4 py-3">
                  <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-app-baunilha-dourada">Conversas</span>
                  <strong className="mt-1 block text-2xl">{conversas.length}</strong>
                </div>
                <div className="rounded-[12px] bg-white/10 px-4 py-3">
                  <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-app-baunilha-dourada">Não lidas</span>
        ...
        <div className="mt-8">
          {carregando ? (
            ...
            <div className="flex min-h-[340px] flex-col items-center justify-center rounded-[18px] bg-white px-6 py-12 text-center shadow-sm ring-1 ring-app-baunilha-dourada/45">
              ...
              <h2 className="mt-5 text-xl font-semibold text-app-cafe-profundo">Nenhuma conversa disponível</h2>
              <p className="mt-2 max-w-md text-sm leading-6 text-app-cinza">Abra o perfil de um restaurante para iniciar uma conversa segura.</p>
            </div>
          )}
        </div>
      </section>
