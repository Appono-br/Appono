...
export default function AvaliarPedidoPage({ params }) {
    ...
    if (!dados && !erro) return <main className="min-h-screen bg-app-chantilly p-8"><div className="mx-auto h-64 max-w-2xl animate-pulse rounded-[16px] bg-app-creme-leve" /></main>;
    return <main className="min-h-screen bg-app-chantilly px-5 py-10 text-app-cafe-profundo"><section className="mx-auto max-w-2xl">
        ...
        {dados && !dados.elegivel ? <section className="mt-6 rounded-[14px] bg-app-creme-leve p-7 ring-1 ring-app-baunilha-dourada/60"><h2 className="text-xl font-semibold">Avaliação ainda indisponível</h2><p className="mt-2 text-sm text-app-cinza">Ela será liberada quando o restaurante marcar o pedido como entregue.</p></section> : null}
        {dados?.elegivel ? <form onSubmit={salvar} className="mt-6 rounded-[14px] bg-app-creme-leve p-7 shadow-sm ring-1 ring-app-baunilha-dourada/60"><fieldset><legend className="text-sm font-bold">Sua nota</legend><div className="mt-4 flex gap-2" aria-label="Nota de 1 a 5 estrelas">{[1, 2, 3, 4, 5].map((valor) => <button key={valor} type="button" onClick={() => setNota(valor)} aria-label={`${valor} estrela${valor > 1 ? "s" : ""}`} className={`text-4xl transition ${valor <= nota ? "text-app-dourado-mel" : "text-app-baunilha-dourada hover:text-app-dourado-mel"}`}>★</button>)}</div><p className="mt-2 text-sm text-app-cinza">{nota ? `${nota} de 5 estrelas` : "Selecione uma nota"}</p></fieldset><label className="mt-6 grid gap-2 text-sm font-bold" htmlFor="comentario-avaliacao">Conte como foi<textarea id="comentario-avaliacao" maxLength={1000} rows={6} value={comentario} onChange={(event) => setComentario(event.target.value)} className="rounded-[10px] border border-app-baunilha-dourada bg-app-chantilly p-4 font-normal outline-none focus:border-app-caramelo-torrado" placeholder="Comente sobre atendimento, ambiente e qualidade do pedido."/></label><div className="mt-2 text-right text-xs text-app-cinza">{comentario.length}/1000</div><button disabled={salvando || !nota} className="mt-5 h-12 w-full rounded-[8px] bg-app-dourado-mel text-xs font-bold uppercase tracking-[0.12em] text-white disabled:opacity-50">{salvando ? "Publicando..." : dados.avaliacao ? "Atualizar avaliação" : "Publicar avaliação"}</button>{salvo ? <p className="mt-4 text-center text-sm font-bold text-green-700">Avaliação publicada com sucesso.</p> : null}</form> : null}
    </section></main>;
}
