...
export default function AvaliarPedidoPage({ params }) {
    ...
    return <main className="min-h-screen bg-white px-5 py-10 text-app-cafe-profundo"><section className="mx-auto max-w-2xl">
        <Link href={`/cliente/pedidos/${id}`} className="text-sm font-bold text-app-caramelo-torrado">{ui("← Voltar ao pedido")}</Link>
        ...
        {dados?.elegivel ? <form onSubmit={salvar} className="mt-6 rounded-[14px] bg-white p-7 shadow-sm ring-1 ring-app-baunilha-dourada/60"><fieldset><legend className="text-sm font-bold">{ui("Sua nota")}</legend><div className="mt-4 flex gap-2" aria-label={ui("Nota de 1 a 5 estrelas")}>{[1, 2, 3, 4, 5].map((valor) => <button key={valor} type="button" onClick={() => setNota(valor)} aria-label={ui("{0} estrela{1}", [valor, valor > 1 ? "s" : ""])} className={`text-4xl transition ${valor <= nota ? "text-app-dourado-mel" : "text-app-baunilha-dourada hover:text-app-dourado-mel"}`}>★</button>)}</div><p className="mt-2 text-sm text-app-cinza">{nota ? ui("{0} de 5 estrelas", [nota]) : ui("Selecione uma nota")}</p></fieldset><label className="mt-6 grid gap-2 text-sm font-bold" htmlFor="comentario-avaliacao">{ui("Conte como foi")}<textarea id="comentario-avaliacao" maxLength={1000} rows={6} value={comentario} onChange={(event) => setComentario(event.target.value)} className="rounded-[10px] border border-app-baunilha-dourada bg-white p-4 font-normal outline-none focus:border-app-caramelo-torrado" placeholder={ui("Comente sobre atendimento, ambiente e qualidade do pedido.")}/></label><div className="mt-2 text-right text-xs text-app-cinza">{comentario.length}/1000</div><button disabled={salvando || !nota} className="mt-5 h-12 w-full rounded-[8px] bg-app-dourado-mel text-xs font-bold uppercase tracking-[0.12em] text-white disabled:opacity-50">{ui(salvando ? "Publicando..." : dados.avaliacao ? "Atualizar avaliação" : "Publicar avaliação")}</button>{salvo ? <p className="mt-4 text-center text-sm font-bold text-green-700">{ui("Avaliação publicada com sucesso.")}</p> : null}</form> : null}
    </section></main>;
}
