...
export default function PaginaPedidoAntecipado({ params, }) {
    ...
    const produtos = useMemo(() => dados?.cardapios.flatMap((cardapio) => (cardapio.categorias ?? []).flatMap((categoria) => categoria.produtos ?? [])) ?? [], [dados]);
    const total = useMemo(() => produtos.reduce((soma, produto) => soma + Number(produto.preco) * (quantidades[produto.id_produto] ?? 0), 0), [produtos, quantidades]);
    ...
    return (<main className="min-h-screen bg-app-chantilly px-5 py-8 text-app-cafe-profundo">
      <div className="mx-auto max-w-6xl">
        ...
        {pedidoAtivo ? (<section className="mt-6 rounded-[12px] bg-app-creme-leve p-8 text-center shadow-sm ring-1 ring-app-baunilha-dourada">
            ...
            <div className="mx-auto mt-6 grid max-w-lg gap-2 text-left">
              {pedidoAtivo.itens_pedido?.map((item, indice) => (<div key={`${item.produtos?.nome ?? "item"}-${indice}`} className="flex justify-between rounded-[8px] bg-app-creme-suave px-4 py-3 text-sm">
          ...
          </section>) : (<div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
            <section className="grid gap-6">
              {dados.cardapios.map((cardapio) => (<article key={cardapio.id_cardapio} className="rounded-[12px] bg-app-creme-leve p-6 shadow-sm ring-1 ring-app-baunilha-dourada">
                  ...
                  <div className="mt-5 grid gap-4">
                    {(cardapio.categorias ?? []).flatMap((categoria) => (categoria.produtos ?? []).map((produto) => (<div key={produto.id_produto} className="grid gap-4 rounded-[8px] bg-app-creme-suave p-4 sm:grid-cols-[1fr_auto] sm:items-center">
                          <div>
              ...
              {!produtos.length ? (<p className="rounded-[12px] bg-app-creme-leve p-8 text-center text-sm font-semibold shadow-sm ring-1 ring-app-baunilha-dourada">
