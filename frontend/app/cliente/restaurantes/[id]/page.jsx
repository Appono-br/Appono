...
export default function PaginaRestaurante({ params, }) {
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            apiRequest(`/restaurantes/${restauranteId}/minha-avaliacao`).catch(() => null),
        ])
            .then(([dadosRestaurante, dadosCardapio, minhaAvaliacao]) => {
            setRestaurante(dadosRestaurante);
            setCardapios(dadosCardapio ?? []);
            if (minhaAvaliacao) { setNota(minhaAvaliacao.nota); setComentario(minhaAvaliacao.comentario ?? ""); }
            setMensagem("");
        })
    ...
    async function enviarAvaliacao(event) {
        event.preventDefault();
        setAvaliando(true); setMensagem("");
        try {
            const resposta = await apiRequest(`/restaurantes/${restauranteId}/avaliacoes`, { method: "POST", body: JSON.stringify({ nota, comentario }) });
            setRestaurante((atual) => ({ ...atual, ...resposta, avaliacoes_recentes: [resposta.avaliacao, ...(atual.avaliacoes_recentes ?? []).filter((item) => item.id_avaliacao !== resposta.avaliacao.id_avaliacao)] }));
            setMensagem("Avaliacao salva com sucesso.");
        } catch (erro) { setMensagem(erro instanceof Error ? erro.message : "Nao foi possivel salvar a avaliacao."); }
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.72fr]">
          ...
          <section className="rounded-[12px] bg-app-creme-leve p-6 shadow-sm ring-1 ring-app-baunilha-dourada">
            ...
            <form onSubmit={enviarAvaliacao} className="mt-5 rounded-[10px] bg-app-creme-suave p-4">
              <label className="grid gap-2 text-xs font-bold uppercase">Sua nota<select value={nota} onChange={(event) => setNota(Number(event.target.value))} className="h-11 rounded-[8px] border border-app-baunilha-dourada bg-app-chantilly px-3 text-sm normal-case">{[5,4,3,2,1].map((valor) => <option key={valor} value={valor}>{valor} estrela(s)</option>)}</select></label>
              <label className="mt-4 grid gap-2 text-xs font-bold uppercase">Comentário<textarea maxLength={1000} value={comentario} onChange={(event) => setComentario(event.target.value)} className="min-h-24 rounded-[8px] border border-app-baunilha-dourada bg-app-chantilly p-3 text-sm font-normal normal-case" /></label>
              <button disabled={avaliando} className="mt-4 rounded-[8px] bg-app-caramelo-torrado px-5 py-3 text-xs font-bold uppercase text-white disabled:opacity-50">{avaliando ? "Salvando..." : "Salvar avaliação"}</button>
            </form>
            <div className="mt-5 grid gap-3">{(restaurante.avaliacoes_recentes ?? []).map((avaliacao) => <article key={avaliacao.id_avaliacao} className="rounded-[10px] bg-app-chantilly p-4 ring-1 ring-app-baunilha-dourada/60"><p className="font-bold text-app-caramelo-torrado">{avaliacao.nota}/5</p><p className="mt-2 text-sm text-app-mocha">{avaliacao.comentario || "Sem comentário."}</p><p className="mt-2 text-xs text-app-cinza">{avaliacao.clientes?.nome ?? "Cliente Appono"}</p></article>)}</div>
          </section>
