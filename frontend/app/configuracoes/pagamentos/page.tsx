...
type PaymentKind = "credit" | "debit" | "pix" | "meal";
...
function Icon({
  ...
  type: "arrow-left" | "card" | "pix" | "voucher";
  className?: string;
}) {
  const paths = {
...
export default function PaymentSettingsPage() {
  ...
  function updateField<Key extends keyof PaymentForm>(
    field: Key,
  ...
  return (
    <main className="flex min-h-screen flex-col bg-app-chantilly text-app-cafe-profundo">
      ...
      <section className="mx-auto w-full max-w-6xl flex-1 px-5 py-10 sm:py-14">
        ...
        >
          <fieldset
            aria-labelledby="payment-type-title"
            className="grid gap-3 rounded-[8px] bg-app-creme-leve p-5 pt-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-6 sm:pt-7"
          >
            <div
              id="payment-type-title"
              className="pb-1 text-xs font-bold uppercase tracking-[0.2em] text-app-mocha"
            >
              Tipo de pagamento
            </div>
            {paymentOptions.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="mt-3 sm:mt-0">
                    ...
                    <span
                      className={`mt-1 block text-xs leading-4 ${
                        isSelected ? "text-app-creme-suave" : "text-app-cinza"
                      }`}
                    >
                      {option.description}
                    </span>
                  ...
                  <span
                    className={`mt-3 h-4 w-4 rounded-full border-2 sm:mt-0 ${
                      isSelected
          ...
          <section className="rounded-[8px] bg-app-chantilly p-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-8">
            <div className="flex items-start justify-between gap-4">
              ...
              {selectedOption ? (
                <span className="flex h-10 w-10 items-center justify-center rounded-[8px] bg-app-baunilha-dourada text-app-caramelo-torrado">
            ...
            <div className="mt-7 grid gap-5 sm:grid-cols-2">
              <Field label="Apelido" className="sm:col-span-2">
                <input
                  ...
                  className="h-12 rounded-[8px] border border-app-baunilha-dourada bg-app-creme-leve px-4 text-sm outline-none transition placeholder:text-app-cinza/50 focus:border-app-dourado-mel focus:ring-2 focus:ring-app-dourado-mel/20"
                />
              </Field>
                ...
                <>
                  <Field label="Nome impresso" className="sm:col-span-2">
                    <input
                      ...
                      className="h-12 rounded-[8px] border border-app-baunilha-dourada bg-app-creme-leve px-4 text-sm outline-none transition focus:border-app-dourado-mel focus:ring-2 focus:ring-app-dourado-mel/20"
                    />
                  </Field>
              ...
              {form.kind === "meal" ? (
                <Field label="Operadora do benefício" className="sm:col-span-2">
                  <input
                    value={form.benefitProvider}
                    onChange={(event) =>
                      updateField("benefitProvider", event.target.value)
                    }
                    className="h-12 rounded-[8px] border border-app-baunilha-dourada bg-app-creme-leve px-4 text-sm outline-none transition focus:border-app-dourado-mel focus:ring-2 focus:ring-app-dourado-mel/20"
                  />
                </Field>
