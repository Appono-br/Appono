...
function Field({ label, value, onChange, className = "", }) {
    return (<label className={`grid gap-2 ${className}`}>
      ...
      <input value={value} onChange={(event) => onChange(event.target.value)} className="h-12 border-b border-app-cinza/45 bg-app-creme-suave px-3 text-base text-app-cafe-profundo outline-none transition focus:border-app-caramelo-torrado"/>
    </label>);
}
export default function RestaurantMenuItemEditorPage() {
    ...
    return (<main className="flex min-h-screen flex-col bg-app-chantilly text-app-cafe-profundo">
      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        ...
        <form onSubmit={submitForm} className="mt-14 grid gap-8 lg:grid-cols-[1fr_0.7fr] lg:items-start">
          <section className="grid gap-8">
            <div className="rounded-[8px] bg-app-creme-leve p-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-8">
              <div className="grid gap-6">
                <Field label="Nome do prato" value={form.name} onChange={(value) => updateField("name", value)}/>
                ...
                <div className="grid gap-6 sm:grid-cols-2">
                  <label className="grid gap-2">
                    ...
                    <span className="relative">
                      <select value={form.category} onChange={(event) => updateField("category", event.target.value)} className="h-12 w-full appearance-none border-b border-app-cinza/45 bg-app-creme-suave px-3 pr-10 text-base text-app-cafe-profundo outline-none transition focus:border-app-caramelo-torrado">
                  ...
                  <Field label="Preco (R$)" value={form.price} onChange={(value) => updateField("price", value)}/>
                </div>
                ...
                <label className="grid gap-2">
                  ...
                  <textarea value={form.description} onChange={(event) => updateField("description", event.target.value)} className="min-h-40 resize-y border-b border-app-cinza/45 bg-app-creme-suave px-3 py-4 text-base leading-7 text-app-cafe-profundo outline-none transition focus:border-app-caramelo-torrado"/>
                </label>
              </div>
            </div>
            ...
            <section className="grid gap-4 sm:grid-cols-[auto_1fr] sm:items-center">
              ...
              <div className="flex flex-wrap gap-4">
                {availabilityOptions.map((option) => (<button key={option.id} type="button" onClick={() => updateField("availability", option.id)} className={`h-11 rounded-[8px] px-8 text-xs font-bold uppercase tracking-[0.16em] transition ${form.availability === option.id
          ...
          <aside className="grid gap-8">
            ...
            const key = field;
            return (<button key={field} type="button" onClick={() => updateField(key, !form[key])} className="inline-flex items-center gap-3 rounded-full bg-app-baunilha-dourada/45 px-4 py-2 text-sm font-semibold text-app-mocha transition hover:bg-app-baunilha-dourada">
                      <span className={`flex h-5 w-5 items-center justify-center rounded-[4px] border border-app-baunilha-dourada bg-app-chantilly ${form[key] ? "text-app-caramelo-torrado" : "text-transparent"}`}>
            ...
            <button type="submit" className="h-14 rounded-[8px] bg-app-dourado-mel px-8 text-sm font-bold uppercase tracking-wide text-white transition hover:bg-app-caramelo-torrado">
              Publicar item
            </button>

            {message ? (<p className="text-sm font-semibold text-app-caramelo-torrado">
                {message}
              </p>) : null}
          </aside>
        </form>
