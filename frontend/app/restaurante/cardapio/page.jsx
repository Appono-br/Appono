...
function EmptyMenuSection({ title }) {
    return (<section className="mt-10">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <Link href="/restaurante/cardapio/editar" className="flex min-h-[280px] flex-col items-center justify-center rounded-[8px] border border-dashed border-app-caramelo-torrado/35 bg-app-chantilly px-6 py-10 text-center transition hover:-translate-y-0.5 hover:bg-app-creme-leve">
    ...
    return (<main className="flex min-h-screen flex-col bg-app-chantilly text-app-cafe-profundo">
      <header className="sticky top-0 z-30 border-b border-app-baunilha-dourada/50 bg-app-creme-leve/90 text-app-cafe-profundo shadow-sm backdrop-blur-md">
        <div className="mx-auto grid h-16 max-w-7xl grid-cols-[1fr_auto_1fr] items-center gap-4 px-5 lg:h-20">
          ...
          <button type="button" onClick={() => setMobileMenuOpen((current) => !current)} className="flex h-9 w-9 items-center justify-center justify-self-end rounded-[8px] border border-app-baunilha-dourada bg-app-chantilly text-app-cafe-profundo xl:hidden" aria-label="Abrir menu" aria-expanded={mobileMenuOpen} aria-controls="restaurant-menu-management">
            <Icon type="menu"/>
          </button>
        </div>

        {mobileMenuOpen ? (<nav id="restaurant-menu-management" className="border-t border-app-baunilha-dourada/55 bg-app-creme-leve px-5 py-3 xl:hidden">
      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        <div className="grid gap-6 border-t border-app-baunilha-dourada/60 pt-10 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button type="button" className="inline-flex h-12 items-center justify-center gap-2 rounded-[8px] bg-app-creme-suave px-6 text-sm font-semibold text-app-mocha transition hover:bg-app-baunilha-dourada">
        ...
        <section className="mt-10 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
          {summaryCards.map((item, index) => (<article key={item} className={`min-h-36 rounded-[8px] p-6 shadow-sm ring-1 ring-app-baunilha-dourada/45 ${index === 2 ? "bg-app-creme-suave" : "bg-app-creme-leve"}`}>
        ...
        {sections.map((section) => (<EmptyMenuSection key={section} title={section}/>))}
