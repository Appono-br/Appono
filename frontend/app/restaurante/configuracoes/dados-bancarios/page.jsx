    ...
    return (<main className="flex min-h-screen flex-col bg-white text-app-cafe-profundo">
      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        <div className="border-t border-app-baunilha-dourada/60 pt-10">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-app-caramelo-torrado">{ui("Repasses e conciliação")}</p>
          <h2 className="mt-3 text-4xl font-medium leading-tight text-app-cafe-profundo sm:text-5xl">{ui("Dados Bancários")}</h2>
          <p className="mt-4 max-w-3xl text-sm leading-6 text-app-cinza sm:text-base">{ui("Cadastre a conta juridica que recebera os repasses. A validação definitiva fica pendente até a integração financeira estar ativa.")}</p>
        </div>
        ...
        <form onSubmit={enviarFormulario} className="mt-10 grid gap-8 lg:grid-cols-[1fr_0.5fr]">
          <section className="rounded-[8px] bg-white p-6 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:p-8">
            <div className="flex flex-col gap-4 border-b border-app-baunilha-dourada/60 pb-7 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h3 className="text-2xl font-medium text-app-cafe-profundo">{ui("Conta de repasse")}</h3>
                <p className="mt-2 text-sm leading-6 text-app-cinza">{ui("Os dados devem pertencer ao mesmo CNPJ do restaurante.")}</p>
              </div>
            ...
            <div className="mt-7 grid gap-5 sm:grid-cols-2">
              <Field label={ui("Razão social titular")} value={form.legalName} onChange={(value) => atualizarCampo("legalName", value)} className="sm:col-span-2" disabled/>
              <Field label={ui("CNPJ titular")} value={form.document} onChange={(value) => atualizarCampo("document", aplicarMascaraCnpj(value))} inputMode="numeric" maxLength={18} disabled/>
              <div className="rounded-[8px] border border-app-baunilha-dourada bg-app-creme-suave p-4 text-sm leading-6 text-app-mocha sm:col-span-2">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-app-cinza">{ui("Dados cadastrados")}</p>
                <div className="mt-2 grid gap-2 sm:grid-cols-3">
                  <span>{ui("Status: ")}<strong>{ui(bankSummary.status === "pendente_validação"
                ? "Pendente de validação"
                : bankSummary.status === "validado"
                    ? "Validado"
                    : "Não configurado")}</strong></span>
                  <span>{ui("Provedor: ")}<strong>{bankSummary.provider}</strong></span>
                  <span>{ui("Atualização: ")}<strong>{ui(bankSummary.updatedAt
                ? new Date(bankSummary.updatedAt).toLocaleDateString("pt-BR")
                : "Sem registro")}</strong></span>
                </div>
                <p className="mt-3 text-xs text-app-cinza">{ui("Por segurança, a Appono não armazena banco, agência, conta ou Pix. No MVP, o envio abaixo apenas muda o status para validação externa pendente.")}</p>
              </div>
              <Field label={ui("Código do banco")} value={form.bankCode} onChange={(value) => atualizarCampo("bankCode", aplicarMascaraCodigoBanco(value))} inputMode="numeric" maxLength={3}/>
              <Field label={ui("Agência")} value={form.agency} onChange={(value) => atualizarCampo("agency", aplicarMascaraAgencia(value))} inputMode="numeric" maxLength={5}/>
              <div className="grid gap-5 sm:grid-cols-[1fr_0.38fr]">
                <Field label={ui("Conta")} value={form.account} onChange={(value) => atualizarCampo("account", somenteNumeros(value).slice(0, 20))} inputMode="numeric" maxLength={20}/>
                <Field label={ui("Digito")} value={form.accountDigit} onChange={(value) => atualizarCampo("accountDigit", value.replace(/[^\dXx]/g, "").slice(0, 1).toUpperCase())} maxLength={1}/>
              </div>
              <label className="grid gap-2">
                <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-app-cinza">{ui("Tipo de conta")}</span>
                <select value={form.accountType} onChange={(event) => atualizarCampo("accountType", event.target.value)} className="h-12 rounded-[8px] border border-app-baunilha-dourada bg-app-creme-suave px-4 text-sm outline-none transition focus:border-app-caramelo-torrado">
                  ...
                  <option value="savings">{ui("Conta poupanca")}</option>
                </select>
              </label>
            </div>
            ...
            <section className="mt-8 border-t border-app-baunilha-dourada/60 pt-8">
              <h3 className="flex items-center gap-3 text-2xl font-medium text-app-cafe-profundo">
                <Icon type="pix" className="h-6 w-6 text-app-caramelo-torrado"/>{ui("Chave Pix de contingencia")}</h3>
              <div className="mt-6 grid gap-5 sm:grid-cols-[0.46fr_1fr]">
                <label className="grid gap-2">
                  <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-app-cinza">{ui("Tipo da chave")}</span>
                  <select value={form.pixKeyType} onChange={(event) => atualizarCampo("pixKeyType", event.target.value)} className="h-12 rounded-[8px] border border-app-baunilha-dourada bg-app-creme-suave px-4 text-sm outline-none transition focus:border-app-caramelo-torrado">
                    ...
                    <option value="random">{ui("Chave aleatoria")}</option>
                  </select>
                </label>
                <Field label={ui("Chave Pix")} value={form.pixKey} onChange={(value) => atualizarCampo("pixKey", form.pixKeyType === "document"
            ...
            <section className="mt-8 border-t border-app-baunilha-dourada/60 pt-8">
              <h3 className="text-2xl font-medium text-app-cafe-profundo">{ui("Frequencia de repasse")}</h3>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-end">
              ...
              <button type="submit" disabled={salvando} className="h-12 rounded-[8px] bg-app-dourado-mel px-8 text-xs font-bold uppercase tracking-wide text-white transition hover:bg-app-caramelo-torrado disabled:cursor-not-allowed disabled:opacity-60">
                {ui(salvando ? "Salvando..." : "Salvar dados")}
              </button>
            </div>
            {message ? <p className="mt-4 text-sm font-semibold text-app-caramelo-torrado">{ui(message)}</p> : null}
          </section>
          ...
          <aside className="grid gap-6">
            <section className="rounded-[8px] bg-app-creme-leve p-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-8">
              ...
              <h3 className="mt-5 text-2xl font-medium text-app-cafe-profundo">{ui("Validação financeira")}</h3>
              <p className="mt-3 text-sm leading-6 text-app-mocha">{ui("Em produção, os dados bancários seriam enviados diretamente para um provedor financeiro. A Appono guardaria apenas o identificador seguro retornado por esse provedor.")}</p>
            </section>

            <section className="rounded-[8px] bg-white p-6 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:p-8">
              <h3 className="flex items-center gap-3 text-xl font-medium text-app-cafe-profundo">
                <Icon type="lock" className="h-5 w-5 text-app-caramelo-torrado"/>{ui("Dados sensiveis")}</h3>
              <p className="mt-3 text-sm leading-6 text-app-cinza">{ui("Banco, agência, conta e Pix não ficam salvos na base da Appono. Essa decisao reduz exposição de dados sensiveis e melhora a aderência a boas práticas de segurança.")}</p>
            </section>

            <section className="rounded-[8px] bg-app-baunilha-dourada p-6 shadow-sm">
              <h3 className="flex items-center gap-3 text-xl font-medium text-app-cafe-profundo">
                <Icon type="card" className="h-5 w-5"/>{ui("Status de repasse")}</h3>
              <div className="mt-5 rounded-[8px] bg-white p-5">
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-app-cinza">{ui("Integração financeira pendente")}</p>
                <p className="mt-2 text-sm leading-6 text-app-mocha">{ui("O histórico de repasses aparecerá aqui quando a integração financeira estiver disponível.")}</p>
              </div>
            </section>
          </aside>
        </form>
