    ...
    return (<main className="flex min-h-screen flex-col bg-white text-app-cafe-profundo">
      ...
      <section className="border-b border-app-baunilha-dourada/45 bg-white px-5 py-7">
        <div className="mx-auto flex max-w-7xl items-center gap-4">
          ...
          <div>
            <h2 className="text-3xl font-medium italic leading-tight text-app-cafe-profundo">{ui("Endereço da Loja")}</h2>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-app-mocha">{ui("Localização fisica e fiscal")}</p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto grid w-full max-w-7xl flex-1 gap-8 px-5 py-10 lg:grid-cols-[1fr_0.7fr] lg:items-start">
        <form onSubmit={submitForm} className="rounded-[8px] bg-white p-6 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:p-10">
          ...
          <div className="mt-8 grid gap-6 sm:grid-cols-2">
            <Field label={ui("Endereço (rua, avenida)")} value={form.street} onChange={(value) => updateField("street", value)} className="sm:col-span-2"/>
            <Field label={ui("Numero")} value={form.number} onChange={(value) => updateField("number", value)}/>
            <Field label={ui("Complemento")} value={form.complement} onChange={(value) => updateField("complement", value)}/>
            <Field label={ui("Bairro")} value={form.district} onChange={(value) => updateField("district", value)} className="sm:col-span-2"/>
            <Field label={ui("Cidade")} value={form.city} onChange={(value) => updateField("city", value)}/>
            <label className="grid gap-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-app-cinza">{ui("Estado")}</span>
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            <button type="submit" disabled={salvando} className="h-12 rounded-[8px] bg-app-dourado-mel px-8 text-xs font-bold uppercase tracking-wide text-white transition hover:bg-app-caramelo-torrado disabled:cursor-not-allowed disabled:opacity-60">
              {ui(salvando ? "Salvando..." : "Salvar endereço")}
            </button>
        ...
        <aside className="overflow-hidden rounded-[8px] bg-app-creme-suave shadow-sm ring-1 ring-app-baunilha-dourada/60">
          <div className="relative flex min-h-[260px] items-center justify-center bg-app-baunilha-dourada/45">
            ...
            <span className="relative inline-flex items-center gap-3 rounded-[8px] bg-white px-6 py-4 text-sm font-bold uppercase tracking-[0.16em] text-app-mocha shadow-sm">
              <Icon type="navigation" className="h-5 w-5 text-app-caramelo-torrado"/>{ui("Mapa preview")}</span>
          </div>
          <div className="p-6">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-app-caramelo-torrado">{ui("Dica de configuração")}</p>
            <p className="mt-3 text-sm leading-6 text-app-mocha">{ui("A posição exata do marcador será ajustada quando o serviço de mapa estiver disponível.")}</p>
          </div>
        </aside>
      </section>
