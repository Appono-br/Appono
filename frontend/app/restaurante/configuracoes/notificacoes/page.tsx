...
type ChannelKey = "email" | "whatsapp" | "push" | "sms";
...
function Icon({
  ...
  type: "arrow-left" | "bell" | "check" | "help" | "mail" | "message" | "phone";
  className?: string;
}) {
  const paths = {
  ...
  return (
    <button
      ...
      className={`relative h-8 w-14 rounded-full transition ${
        checked ? "bg-app-mocha" : "bg-app-cinza/35"
      }`}
      aria-label={label}
    >
      <span
...
export default function RestaurantNotificationSettingsPage() {
  ...
  function toggleChannel(ruleKey: NotificationKey, channel: ChannelKey) {
    setForm((current) => ({
      ...current,
        ...
        <form onSubmit={submitForm} className="mt-10 grid gap-8 lg:grid-cols-[0.72fr_1fr]">
          ...
          <section className="rounded-[8px] bg-app-baunilha-dourada p-5 shadow-sm ring-1 ring-app-caramelo-torrado/15 sm:p-8">
            ...
            <div className="mt-7 grid gap-4">
              {form.rules.map((rule) => (
                <article key={rule.key} className="rounded-[8px] bg-app-chantilly p-5 ring-1 ring-app-baunilha-dourada/50">
                  <div className="grid gap-4 xl:grid-cols-[1fr_auto] xl:items-start">
                    <div>
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {rule.required ? (
                          <span className="rounded-full bg-app-creme-suave px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-app-caramelo-torrado">
                            Critico
                          </span>
                        ) : null}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 xl:min-w-[360px]">
                      {([
                        ...
                        ["push", "App"],
                        ["sms", "SMS"],
                      ] as Array<[ChannelKey, string]>).map(([channel, label]) => (
                        <div key={channel} className="flex items-center justify-between gap-3 rounded-[8px] bg-app-creme-suave px-3 py-2">
                          <span className="text-xs font-bold uppercase text-app-mocha">
                            {label}
                          </span>
                          <Toggle
                            checked={rule.channels[channel]}
                            onChange={() => toggleChannel(rule.key, channel)}
                            label={`${rule.channels[channel] ? "Desativar" : "Ativar"} ${label} em ${rule.title}`}
                          />
                        </div>
                      ))}
                    </div>
