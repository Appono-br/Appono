...
const initialDays: OperationDay[] = [
  ...
  return (
    <label className={`grid gap-2 ${className}`}>
      ...
      <input
        ...
        className="h-12 border-b border-app-baunilha-dourada bg-app-creme-suave px-3 text-sm text-app-cafe-profundo outline-none transition focus:border-app-caramelo-torrado"
      />
    </label>
  );
}
...
export default function RestaurantOperationSettingsPage() {
  ...
  function updateDay(dayId: DayId, updater: (day: OperationDay) => OperationDay) {
    setForm((current) => ({
      ...current,
  ...
  function updateShift(dayId: DayId, index: number, field: keyof Shift, value: string) {
    updateDay(dayId, (day) => ({
      ...day,
            ...
            <section className="rounded-[8px] bg-app-chantilly p-6 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:p-8">
              ...
              <div className="mt-6 grid gap-6 sm:grid-cols-2">
                ...
                <TextField
                  label="ID da loja"
                  value={form.storeId}
                  onChange={(value) => updateField("storeId", value)}
                />
                <TextField
              ...
              <Link
                href="/restaurante/configuracoes/endereco"
                className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-app-caramelo-torrado transition hover:text-app-cafe-profundo"
              >
          ...
          <section className="rounded-[8px] bg-app-baunilha-dourada p-5 shadow-sm ring-1 ring-app-caramelo-torrado/15 sm:p-8">
            ...
            <div className="mt-7 grid gap-4">
              {form.days.map((day) => (
                <article
                  key={day.id}
                  className="grid gap-5 rounded-[8px] border-l-4 border-app-caramelo-torrado bg-app-chantilly p-5 sm:grid-cols-[0.34fr_1fr_auto] sm:items-center"
                >
                  <div>
                    ...
                    {day.shifts.map((shift, index) => (
                      ...
                      >
                        <label className="relative">
                          <Icon
                            type="clock"
                            className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-app-cinza"
                          />
                          <input
                            type="time"
                            value={shift.open}
                            onChange={(event) =>
                              updateShift(day.id, index, "open", event.target.value)
                            }
                            className="h-11 w-full rounded-[8px] border border-app-baunilha-dourada bg-app-creme-suave pl-10 pr-3 text-sm text-app-cafe-profundo outline-none transition focus:border-app-caramelo-torrado"
                          />
                        </label>
                        ...
                        <input
                          ...
                          value={shift.close}
                          onChange={(event) =>
                            updateShift(day.id, index, "close", event.target.value)
                          }
                          className="h-11 w-full rounded-[8px] border border-app-baunilha-dourada bg-app-creme-suave px-3 text-sm text-app-cafe-profundo outline-none transition focus:border-app-caramelo-torrado"
                        />
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-3 sm:justify-end">
                    <button
                      ...
                      onClick={() =>
                        updateDay(day.id, (current) => ({
                          ...current,
                          shifts:
                            current.shifts.length > 1
                              ? current.shifts.slice(0, -1)
                              : current.shifts,
                        }))
                      }
                      className="flex h-9 w-9 items-center justify-center rounded-full bg-app-creme-suave text-app-vermelho-erro transition hover:bg-app-areia-quente"
                      aria-label={`Remover turno de ${day.label}`}
                    >
                    ...
                    <button
                      ...
                      className="flex h-9 w-9 items-center justify-center rounded-full bg-app-creme-suave text-app-cafe-profundo transition hover:bg-app-areia-quente"
                      aria-label={`Adicionar turno em ${day.label}`}
                    >
                    ...
                    <button
                      ...
                      className={`relative h-8 w-14 rounded-full transition ${
                        day.enabled ? "bg-app-mocha" : "bg-app-cinza/35"
                      }`}
                      aria-label={`${day.enabled ? "Desativar" : "Ativar"} ${day.label}`}
                    >
                      <span
                        className={`absolute top-1 h-6 w-6 rounded-full bg-app-chantilly transition ${
            ...
            {message ? (
              <p className="mt-4 text-sm font-semibold text-app-mocha">{message}</p>
            ) : null}
          </section>
        </form>
        ...
        <section className="mt-10 rounded-[8px] bg-app-creme-suave p-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:grid sm:grid-cols-[1fr_0.42fr] sm:gap-8 sm:p-8">
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <Link
                href="#"
                className="rounded-[8px] bg-app-baunilha-dourada px-5 py-3 text-[10px] font-bold uppercase tracking-[0.16em] text-app-mocha transition hover:bg-app-areia-quente"
              >
          ...
          <aside className="mt-6 rounded-[8px] bg-app-chantilly p-6 ring-1 ring-app-baunilha-dourada/45 sm:mt-0">
            ...
            <Link
              href="#"
              className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-app-cafe-profundo transition hover:text-app-caramelo-torrado"
            >
              Falar com suporte
