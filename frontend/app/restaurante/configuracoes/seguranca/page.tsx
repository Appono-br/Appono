...
type Role = "manager" | "kitchen" | "host" | "finance";
...
type AccessForm = {
  ...
  requireTwoFactorForTeam: boolean;
...
function Icon({
  ...
  type: "arrow-left" | "help" | "key" | "lock" | "shield" | "user-plus";
  className?: string;
}) {
  const paths = {
...
export default function RestaurantSecuritySettingsPage() {
  ...
  const [form, setForm] = useState<AccessForm>(() => {
    ...
    return stored ? (JSON.parse(stored) as AccessForm) : initialForm;
  });
  ...
  function updateField<Key extends keyof AccessForm>(field: Key, value: AccessForm[Key]) {
              ...
              <div className="mt-6 grid gap-5">
                <label className="grid gap-2">
                  ...
                  <input
                    ...
                    onChange={(event) => updateField("inviteEmail", event.target.value)}
                ...
                <label className="grid gap-2">
                  ...
                  <select
                    ...
                    onChange={(event) => updateField("inviteRole", event.target.value as Role)}
                    className="h-12 rounded-[8px] border border-app-baunilha-dourada bg-app-creme-suave px-4 text-sm outline-none transition focus:border-app-caramelo-torrado"
                  >
          ...
          <section className="rounded-[8px] bg-app-baunilha-dourada p-5 shadow-sm ring-1 ring-app-caramelo-torrado/15 sm:p-8">
            ...
            <div className="mt-7 grid gap-4">
              {[
                ...
                <article key={item.title} className="flex flex-col gap-4 rounded-[8px] bg-app-chantilly p-5 ring-1 ring-app-baunilha-dourada/50 sm:flex-row sm:items-center sm:justify-between">
                  <div>
