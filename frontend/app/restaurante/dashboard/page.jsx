...
function MetricCard({ metric }) {
    ...
    return (<article className={`rounded-[8px] p-5 shadow-sm ring-1 ring-app-baunilha-dourada/45 ${metric.highlighted ? "bg-app-creme-suave" : "bg-white"}`}>
      ...
      {metric.helper ? (<p className="mt-2 text-sm text-app-mocha">{ui(metric.helper)}</p>) : null}
    </article>);
}
      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.52fr]">
          <article className="rounded-[8px] bg-white p-5 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <h2 className="text-2xl font-medium text-app-cafe-profundo">{ui("Reservas recentes")}</h2>
                <p className="mt-2 text-sm text-app-cinza">{ui("Volume real de reservas dos ultimos 7 dias.")}</p>
              </div>
              <div className="flex gap-4 text-xs font-semibold text-app-mocha">
                <span className="flex items-center gap-2">
                  <span className="h-3 w-3 rounded-full bg-app-cafe-profundo"/>{ui("Reservas")}</span>
              </div>
            </div>
            ...
            <div className="mt-8 flex min-h-[220px] items-end gap-3 rounded-[8px] bg-app-creme-leve px-4 py-5 sm:gap-5">
              {serieReservas.map((ponto) => (
                <div key={ponto.data} className="flex flex-1 flex-col items-center gap-2">
                  ...
                  <span className="text-[10px] font-semibold text-app-cinza">{ui(ponto.label)}</span>
                </div>
              ))}
            </div>
          </article>
          ...
          <article className="rounded-[8px] bg-app-cafe-profundo p-5 text-app-creme-leve shadow-sm sm:p-6">
            <h2 className="text-2xl font-medium">{ui("Cozinha")}</h2>
            <div className="mt-8 grid gap-3">
              {proximosPedidos.length ? proximosPedidos.map((pedido) => (<div key={pedido.id_pedido} className="rounded-[10px] bg-app-cacau-intenso/55 p-4 ring-1 ring-app-baunilha-dourada/20">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <h3 className="mt-2 text-sm font-semibold text-app-creme-leve">
                          {pedido.reserva.clientes?.nome ?? ui("Cliente")}
                        </h3>
                      </div>
                  ...
                  </div>)) : (<EmptyPanel dark title={ui("Nenhum pedido antecipado")} description={ui("Quando o cliente pedir antes da reserva, o pedido aparecerá aqui.")}/>)}
            </div>
            <Link href="/restaurante/pedidos" className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-app-baunilha-dourada transition hover:text-app-chantilly">{ui("Ver cozinha")}<Icon type="arrow-right" className="h-4 w-4"/>
            </Link>
          </article>
        </section>
        ...
        <section className="mt-6 grid gap-6 lg:grid-cols-[0.95fr_1fr]">
          <article className="rounded-[8px] bg-app-creme-leve p-5 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-6">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-app-caramelo-torrado">{ui("Destaque do cardápio")}</p>
            <div className="mt-5">
              {produtoDestaque ? (
                <div className="rounded-[10px] bg-white p-5 ring-1 ring-app-baunilha-dourada/50">
                  ...
                  <p className="mt-2 text-sm leading-6 text-app-cinza">{produtoDestaque.descricao ?? ui("Item destacado no cardápio.")}</p>
                  <strong className="mt-4 block text-app-caramelo-torrado">{formatarMoeda(produtoDestaque.preco, localeUI)}</strong>
                </div>
              ) : (
                <EmptyPanel title={ui("Nenhum item em destaque")} description={ui("Selecione um item para destacar no cardápio.")}/>
              )}
            </div>
          </article>
