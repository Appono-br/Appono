...
function Icon({
  ...
  const paths = {
    ...
    money:
      "M4 7h16v10H4V7z M7 10h.01M17 14h.01M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z",
    orders:
      "M7 3v8M4 3v8M10 3v8M4 11h6M7 11v10M17 3v18M14 3h3a3 3 0 0 1 3 3v5a3 3 0 0 1-3 3h-3",
    seat:
      "M7 13V7a4 4 0 0 1 8 0v6M5 13h14v5H5v-5z M8 18v3M16 18v3",
    user:
      "M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z M19 8v6M22 11h-6",
  };
...
function MetricCard({ metric }: { metric: Metric }) {
  return (
    <article
      className={`min-h-44 rounded-[8px] p-7 shadow-sm ring-1 ring-app-baunilha-dourada/45 ${
        metric.highlighted ? "bg-app-creme-suave" : "bg-app-chantilly"
      }`}
    >
      ...
      {metric.helper ? (
        <p className="mt-2 text-sm text-app-mocha">{metric.helper}</p>
      ) : (
        <div className="mt-4 h-2 w-full max-w-48 rounded-full bg-app-baunilha-dourada/45">
...
function EmptyPanel({
  ...
  return (
    <div
      className={`flex min-h-52 flex-col justify-center rounded-[8px] border border-dashed px-6 py-8 ${
        dark
    ...
    >
      ...
      <p
        className={`mt-3 max-w-md text-sm leading-6 ${
          dark ? "text-app-baunilha-dourada" : "text-app-cinza"
        }`}
      >
        {description}
      </p>
          ...
          <article className="rounded-[8px] bg-app-chantilly p-6 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:p-8">
            ...
            <div className="mt-10 flex min-h-[280px] items-end gap-3 rounded-[8px] border border-dashed border-app-caramelo-torrado/25 bg-app-creme-leve px-5 py-6 sm:gap-5">
              {Array.from({ length: 7 }).map((_, index) => (
                <div
                  key={index}
                  className="h-20 flex-1 rounded-t-[8px] bg-app-baunilha-dourada/45"
                />
              ))}
            </div>
          </article>
          ...
          <article className="rounded-[8px] bg-app-cafe-profundo p-6 text-app-creme-leve shadow-sm sm:p-8">
            ...
            <div className="mt-8">
              <EmptyPanel
                ...
                description="A agenda do restaurante aparecera aqui."
              />
            </div>
            <Link
              href="#"
              className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-app-baunilha-dourada transition hover:text-app-chantilly"
            >
              Ver agenda completa
        ...
        <section className="mt-8 grid gap-8 lg:grid-cols-[0.95fr_1fr]">
          <article className="rounded-[8px] bg-app-creme-leve p-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-8">
            ...
            <div className="mt-5">
              <EmptyPanel
                ...
                description="Selecione um item para destacar no cardapio."
              />
            </div>
          </article>
          ...
          <article className="rounded-[8px] bg-app-creme-leve p-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-8">
            ...
            <div className="mt-8 grid gap-6">
              {["Tempo medio de preparo", "Satisfacao do cliente"].map(
                (label) => (
                  <div key={label}>
