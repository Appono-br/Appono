...
function FinanceCard({
  ...
  return (
    <article
      className={`min-h-48 rounded-[8px] p-7 shadow-sm ${
        featured
    ...
    >
      <p
        className={`text-xs font-bold uppercase tracking-[0.18em] ${
          featured ? "text-app-baunilha-dourada" : "text-app-cinza"
        }`}
      >
        {label}
      </p>
      ...
      <div
        className={`mt-8 h-2 max-w-56 rounded-full ${
          featured ? "bg-app-mocha" : "bg-app-baunilha-dourada/45"
        }`}
      >
...
function EmptyTable() {
  return (
    <div className="overflow-hidden rounded-[8px] bg-app-chantilly shadow-sm ring-1 ring-app-baunilha-dourada/45">
      ...
      <header className="sticky top-0 z-30 border-b border-app-baunilha-dourada/50 bg-transparent text-app-cafe-profundo backdrop-blur-sm">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:h-24">
          ...
          <button
            ...
            aria-controls="restaurant-finance-menu"
          >
            <Icon type="menu" />
          </button>
        </div>
      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        <div className="grid gap-6 border-t border-app-baunilha-dourada/60 pt-10 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button
              type="button"
              className="inline-flex h-11 items-center justify-center gap-2 rounded-[8px] bg-app-creme-suave px-5 text-sm font-semibold text-app-mocha transition hover:bg-app-baunilha-dourada"
            >
          ...
          <article className="rounded-[8px] bg-app-creme-suave p-6 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:p-8">
            ...
            <div className="mt-8 flex min-h-44 items-end gap-4 rounded-[8px] border border-dashed border-app-caramelo-torrado/25 bg-app-creme-leve px-5 py-6">
              {Array.from({ length: 5 }).map((_, index) => (
                <div
                  key={index}
                  className="h-16 flex-1 rounded-t-[8px] bg-app-baunilha-dourada/45"
                />
              ))}
            </div>
