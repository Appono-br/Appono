...
const dailySummary: Metric[] = [
  { label: "Reservas hoje" },
  { label: "Mesas reservadas" },
];

const monthlyPerformance: Metric[] = [
  ...
  return (
    <div>
      <div className="flex items-end gap-3">
        ...
        {metric.suffix ? (
          <span className="pb-2 text-sm font-bold text-green-700">
            {metric.suffix}
          </span>
        ) : null}
        {metric.status ? (
          <span className="pb-2 text-sm font-bold text-app-mocha">
            {metric.status}
          </span>
        ) : null}
      </div>
...
function PerformanceCard({ metric, index }: { metric: Metric; index: number }) {
  return (
    <article
      className={`min-h-44 rounded-[8px] bg-app-creme-leve px-6 py-7 shadow-sm ring-1 ring-app-baunilha-dourada/60 transition hover:-translate-y-0.5 hover:bg-app-chantilly ${
        index % 2 === 0
    ...
    <main className="flex min-h-screen flex-col bg-app-chantilly text-app-cafe-profundo">
      <header className="sticky top-0 z-30 border-b border-app-baunilha-dourada/50 bg-app-creme-leve/90 text-app-cafe-profundo shadow-sm backdrop-blur-md">
        <div className="mx-auto grid h-16 max-w-7xl grid-cols-[1fr_auto_1fr] items-center gap-4 px-5 lg:h-20">
          ...
          <button
            ...
            aria-controls="restaurant-home-menu"
          >
            <Icon type="menu" />
          </button>
        </div>
      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        ...
        <section className="mt-8 max-w-4xl rounded-[8px] bg-app-creme-leve px-6 py-8 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:px-8">
          ...
          <Link
            href="#"
            className="mt-12 inline-flex items-center gap-1 text-sm font-bold text-app-caramelo-torrado transition hover:text-app-cafe-profundo"
          >
            Gerenciar Sala
