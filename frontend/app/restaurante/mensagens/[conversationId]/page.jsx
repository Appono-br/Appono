    ...
    <main className="flex min-h-screen flex-col bg-white text-app-cafe-profundo">
      ...
      <section className="mx-auto grid w-full max-w-7xl flex-1 gap-6 px-5 py-8 xl:grid-cols-[1fr_340px]">
        <section className="flex min-h-[680px] flex-col">
          <div className="mb-5 rounded-[18px] bg-app-cafe-profundo p-5 text-app-creme-leve shadow-sm">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <button type="button" onClick={() => setConfirmarLimpeza(true)} className="w-fit rounded-[8px] border border-app-baunilha-dourada/35 px-4 py-2 text-[10px] font-bold uppercase tracking-[0.12em] text-app-creme-leve transition hover:bg-white/10">{ui("Limpar histórico")}</button>
            </div>
          </div>
          ...
          <div className="flex-1 space-y-4 rounded-[18px] bg-white p-4 shadow-sm ring-1 ring-app-baunilha-dourada/45 sm:p-6">
            {mensagens.length ? mensagens.map((item) => {
              ...
              <div className="flex min-h-[360px] flex-col items-center justify-center text-center">
                ...
                <h2 className="mt-5 text-xl font-semibold">{ui("Conversa sem mensagens")}</h2>
                <p className="mt-2 max-w-md text-sm leading-6 text-app-cinza">{ui("Responda o cliente por aqui. A conversa fica vinculada ao perfil correto.")}</p>
              </div>
            )}
            <div ref={fimRef} />
          </div>
        ...
        <aside className="h-fit rounded-[18px] bg-white p-5 shadow-sm ring-1 ring-app-baunilha-dourada/45">
          <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-app-caramelo-torrado">{ui("Contexto")}</p>
          <h2 className="mt-2 text-2xl font-semibold">{ui("Atendimento")}</h2>
          <div className="mt-5 flex items-center gap-4 rounded-[14px] bg-white p-3 ring-1 ring-app-baunilha-dourada/55">
            ...
            <div className="min-w-0">
              <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-app-cinza">{ui("Restaurante")}</span>
              <strong className="mt-1 block truncate text-app-cafe-profundo">{conversa?.restaurante?.nome ?? ui("Restaurante")}</strong>
            </div>
          </div>
          <div className="mt-5 grid gap-3 text-sm">
            <div className="rounded-[12px] bg-app-chantilly p-4">
              <span className="text-xs font-bold uppercase tracking-[0.14em] text-app-cinza">{ui("Cliente")}</span>
              <strong className="mt-1 block">{conversa?.cliente?.nome ?? conversa?.titulo ?? ui("--")}</strong>
            </div>
            <div className="rounded-[12px] bg-app-chantilly p-4">
              <span className="text-xs font-bold uppercase tracking-[0.14em] text-app-cinza">{ui("Reserva")}</span>
              <strong className="mt-1 block">
                {conversa?.reserva ? ui("{0} às {1}", [formatarData(conversa.reserva.data_reserva, localeUI), String(conversa.reserva.horario_inicio).slice(0, 5)]) : ui("Sem reserva vinculada")}
              </strong>
            </div>
            <div className="rounded-[12px] bg-app-chantilly p-4">
              <span className="text-xs font-bold uppercase tracking-[0.14em] text-app-cinza">{ui("Pedido")}</span>
              <strong className="mt-1 block">{conversa?.pedido ? ui("#{0} - {1}", [conversa.pedido.id_pedido, conversa.pedido.status_pedido]) : ui("Sem pedido vinculado")}</strong>
            </div>
          </div>
          <p className="mt-5 flex items-start gap-2 text-xs leading-5 text-app-cinza">
            <Icon type="file" className="mt-0.5 h-4 w-4 shrink-0" />{ui("As mensagens são restritas ao cliente e ao restaurante envolvidos.")}</p>
        </aside>
      </section>
      <ConfirmationDialog
        ...
        description={ui("A conversa será ocultada apenas para este restaurante. O cliente continuará com o próprio histórico e os registros seguem preservados.")}
