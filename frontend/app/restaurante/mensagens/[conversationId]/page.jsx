...
export default function RestaurantConversationPage() {
    ...
    const conversation = useMemo(() => conversations.find((item) => item.id === params.conversationId) ??
        emptyConversation, [params.conversationId]);
    ...
    return (<main className="flex min-h-screen flex-col bg-app-chantilly text-app-cafe-profundo">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="relative flex h-10 w-10 items-center justify-center rounded-[8px] bg-app-mocha text-xs font-bold ring-1 ring-app-baunilha-dourada/30">
              ...
              {conversation.online ? (<span className="absolute -bottom-1 -right-1 h-3 w-3 rounded-full bg-emerald-500"/>) : null}
            </span>
            <div>
      ...
      <section className="mx-auto grid w-full max-w-7xl flex-1 gap-8 px-5 py-10 xl:grid-cols-[1fr_0.72fr]">
        <section className="flex min-h-[720px] flex-col">
          {messages.length ? (<div className="mb-8 flex justify-center">
          ...
          <div className="flex-1 space-y-8">
            {messages.length ? (messages.map((message) => (<article key={message.id} className={`flex gap-4 ${message.sender === "restaurant" ? "justify-end" : "justify-start"}`}>
                {message.sender === "client" ? (<span className="mt-auto flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-app-creme-suave text-[10px] font-bold text-app-mocha ring-1 ring-app-baunilha-dourada">
                    {conversation.initials}
                  </span>) : null}
              ...
              </article>))) : (<div className="flex min-h-[420px] flex-col items-center justify-center rounded-[8px] border border-dashed border-app-caramelo-torrado/30 bg-app-creme-leve px-6 text-center">
          ...
          <form onSubmit={submitMessage} className="mt-8 flex items-center gap-3 rounded-[8px] bg-app-chantilly p-4 shadow-sm ring-1 ring-app-baunilha-dourada/60">
            ...
            <input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Escreva sua mensagem aqui..." className="h-12 min-w-0 flex-1 bg-transparent px-2 text-sm outline-none placeholder:text-app-cinza/60"/>
            <button type="submit" className="flex h-12 w-14 items-center justify-center rounded-[8px] bg-app-areia-quente text-app-cafe-profundo transition hover:bg-app-dourado-mel hover:text-white" aria-label="Enviar mensagem">
              <Icon type="send"/>
            </button>
          </form>
        </section>
