      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
        <div className="relative isolate overflow-hidden rounded-[30px_76px_30px_76px] bg-app-cafe-profundo text-app-creme-leve shadow-sm">
          <div className="relative px-7 py-8 sm:px-10 sm:py-10">
            <h1 className="text-4xl font-semibold leading-[0.92] tracking-[-0.05em] sm:text-6xl">{ui("Mensagens recebidas")}</h1>
            ...
            <div className="mt-8 flex flex-wrap gap-x-9 gap-y-4 border-t border-white/10 pt-5">
              ...
              <div className="flex items-baseline gap-3">
                <strong className="text-3xl leading-none">{totalComPedido}</strong>
        ...
        <label className="campo-busca-app mt-5 flex h-12 items-center gap-2 rounded-full bg-white px-3 text-app-cafe-profundo shadow-sm ring-1 ring-app-baunilha-dourada/60 transition focus-within:ring-app-caramelo-torrado sm:gap-3 sm:px-4">
          ...
          <input value={busca} onChange={(event) => setBusca(event.target.value)} placeholder={ui("Buscar cliente ou mensagem")} className="input-busca-app h-full min-w-0 flex-1 bg-transparent text-base outline-none placeholder:text-app-cinza/70" />
        </label>

        <div className="mt-3 flex flex-wrap justify-end gap-2">
          {filtros.map((item) => (
            <button key={item.id} type="button" onClick={() => setFiltro(item.id)} className={`h-10 rounded-full px-5 text-[10px] font-bold uppercase tracking-[0.14em] transition ${filtro === item.id ? "bg-app-cafe-profundo text-app-creme-leve" : "border border-app-baunilha-dourada bg-white text-app-mocha hover:border-app-caramelo-torrado hover:text-app-caramelo-torrado"}`}>
              {ui(item.label)}
            </button>
          ))}
        </div>
        ...
        <section className="mt-8 grid gap-4">
          {carregando ? (
            ...
            <div className="flex flex-col items-center px-4 py-8 text-center">
              <span className="text-app-caramelo-torrado">
                <Icon type="message" />
              </span>
              <h2 className="mt-3 text-base font-semibold">{ui("Nenhuma conversa recebida")}</h2>
              <p className="mt-1 max-w-md text-sm leading-6 text-app-cinza">{ui("Quando um cliente chamar o restaurante, a conversa aparecerá aqui.")}</p>
            </div>
          )}
        </section>
      </section>
