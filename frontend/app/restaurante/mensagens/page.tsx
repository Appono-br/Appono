...
type ConversationStatus = "vip" | "reserva" | "duvida";
...
function Icon({
  ...
  type: "chevron-right" | "menu" | "message" | "plus";
  className?: string;
}) {
  const paths = {
...
function StatusBadge({ status }: { status: ConversationStatus }) {
  ...
  return (
    <span className="rounded-[4px] bg-app-creme-suave px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-app-mocha">
      {label}
    </span>
  );
}
      ...
      <section className="mx-auto w-full max-w-7xl flex-1 px-5 py-10 sm:py-14">
          ...
          <div className="grid overflow-hidden rounded-[8px] bg-app-creme-leve p-1 shadow-sm ring-1 ring-app-baunilha-dourada/60 sm:grid-cols-3">
            {filterItems.map((item) => (
              <button
                ...
                className={`h-10 rounded-[8px] px-7 text-[10px] font-bold uppercase tracking-[0.22em] transition ${
                  filter === item.id
        ...
        <section className="mt-10 grid gap-5">
          {visibleConversations.length ? (
            visibleConversations.map((conversation) => (
              ...
              >
                <span className="relative flex h-16 w-16 items-center justify-center rounded-[8px] bg-app-creme-suave text-lg font-bold text-app-mocha ring-1 ring-app-baunilha-dourada">
                  ...
                  {conversation.online ? (
                    <span className="absolute -bottom-1 -right-1 h-4 w-4 rounded-full border-2 border-app-chantilly bg-emerald-500" />
                  ) : null}
                </span>
                <span>
                ...
                <span className="flex items-center justify-between gap-5 text-sm font-semibold text-app-cinza sm:block sm:text-right">
                  ...
                  {conversation.unread ? (
                    <span className="mt-2 block h-2.5 w-2.5 rounded-full bg-app-caramelo-torrado sm:ml-auto" />
                  ) : (
                    <span className="mt-2 block text-app-baunilha-dourada">
            ...
            <div className="flex min-h-[320px] flex-col items-center justify-center rounded-[8px] bg-app-chantilly px-6 text-center shadow-sm ring-1 ring-app-baunilha-dourada/45">
        ...
        <button
          type="button"
          className="fixed bottom-8 right-8 flex h-16 w-16 items-center justify-center rounded-[8px] bg-app-areia-quente text-app-cafe-profundo shadow-lg transition hover:-translate-y-0.5 hover:bg-app-dourado-mel hover:text-white"
