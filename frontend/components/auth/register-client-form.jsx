...
export function RegisterClientForm() {
    ...
    return (<form onSubmit={enviarFormulario} className="mx-auto w-full max-w-xl">
      <div className="rounded-[12px] bg-app-chantilly px-5 py-4 shadow-[0_18px_50px_rgba(74,44,10,0.08)] ring-1 ring-app-baunilha-dourada sm:px-7">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <FormField label="Nome completo" value={form.name} onChange={(event) => atualizarCampo("name", event.target.value)} placeholder="Ex: Maria Silva" required className="sm:col-span-2"/>
          <FormField label="Data de nascimento" type="date" value={form.birthDate} onChange={(event) => atualizarCampo("birthDate", event.target.value)} required/>
          <FormField label="CPF" value={form.cpf} onChange={(event) => atualizarCampo("cpf", aplicarMascaraCpf(event.target.value))} onBlur={async () => {
            if (!cpfEstaCompleto(form.cpf)) {
                return;
            }
            try {
          ...
          <FormField label="E-mail" type="email" value={form.email} onChange={(event) => atualizarCampo("email", event.target.value)} placeholder="maria@exemplo.com" required className="sm:col-span-2"/>
          <FormField label="Telefone" value={form.phone} onChange={(event) => atualizarCampo("phone", aplicarMascaraTelefone(event.target.value))} placeholder="(11) 99999-9999" inputMode="tel" maxLength={15} required className="sm:col-span-2"/>
          <FormField label="Senha" type="password" value={form.password} onChange={(event) => atualizarCampo("password", event.target.value)} placeholder="********" required minLength={6} className="sm:col-span-2"/>
        </div>
        ...
        <div className="mt-3 flex flex-col-reverse gap-2 border-t border-app-baunilha-dourada pt-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-xs text-app-cinza">
            Ja possui uma conta?{" "}
            <Link href="/login" className="font-bold text-app-caramelo-torrado transition hover:text-app-cafe-profundo">
              Entrar
            </Link>
          </p>
          <button type="submit" disabled={isSubmitting} className="flex h-9 w-full items-center justify-center bg-app-dourado-mel px-6 text-xs font-bold uppercase tracking-wide text-white transition hover:-translate-y-0.5 hover:bg-app-caramelo-torrado focus:outline-none focus:ring-4 focus:ring-app-dourado-mel/25 disabled:cursor-not-allowed disabled:opacity-70 sm:w-auto">
            {isSubmitting ? "Criando..." : "Criar conta"}
          </button>
        </div>

        {message ? (<p className="mt-2 text-sm font-semibold text-app-caramelo-torrado">
            {message}
          </p>) : null}
      </div>
    </form>);
