...
import { aplicarMascaraAgencia, aplicarMascaraCodigoBanco, aplicarMascaraConta, } from "@/lib/validacoes/dados-bancarios";
    ...
    return (<div className="mx-auto w-full max-w-6xl">
      <div className="rounded-[12px] bg-app-chantilly px-5 py-4 shadow-[0_18px_50px_rgba(74,44,10,0.08)] ring-1 ring-app-baunilha-dourada sm:px-7">
        <div className="mb-3 flex justify-center">
          <Image src="/brand/appono-mark.svg" alt="Appono" width={108} height={108} className="h-16 w-16" priority/>
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <div className="grid grid-cols-2 overflow-hidden border border-app-baunilha-dourada text-center text-[10px] font-bold uppercase tracking-[0.18em]">
            <button type="button" onClick={() => setStep(1)} className={`px-4 py-2 transition ${step === 1
            ? "bg-app-cafe-profundo text-app-creme-leve"
            : "bg-app-creme-suave text-app-caramelo-torrado hover:bg-app-baunilha-dourada"}`}>
              Etapa 1
            </button>
            <button type="button" onClick={irParaEtapaBancaria} className={`px-4 py-2 transition ${step === 2
            ? "bg-app-cafe-profundo text-app-creme-leve"
            : "bg-app-creme-suave text-app-caramelo-torrado hover:bg-app-baunilha-dourada"}`}>
              Etapa 2
            </button>
        ...
        {step === 1 ? (<div className="mt-3 grid gap-1.5 sm:grid-cols-6 xl:grid-cols-12">
            ...
            <label className="grid gap-1 rounded-[8px] border border-dashed border-app-caramelo-torrado/50 bg-app-creme-suave p-3 sm:col-span-6 xl:col-span-4">
              ...
              <span className="flex items-center gap-3">
                <span className="h-12 w-12 shrink-0 rounded-[8px] bg-app-baunilha-dourada bg-cover bg-center" style={imagemPreview ? { backgroundImage: `url("${imagemPreview}")` } : undefined}/>
              ...
              <input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => selecionarImagem(event.target.files?.[0])} className="text-xs text-app-mocha file:mr-3 file:border-0 file:bg-app-caramelo-torrado file:px-3 file:py-2 file:text-xs file:font-bold file:text-white"/>
            </label>
          </div>) : (<div className="mt-3 grid gap-2 sm:grid-cols-6">
        ...
        <div className="mt-3 flex flex-col-reverse gap-2 border-t border-app-baunilha-dourada pt-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {step === 1 ? (<button type="button" onClick={irParaEtapaBancaria} className="flex h-9 w-full items-center justify-center bg-app-dourado-mel px-6 text-xs font-bold uppercase tracking-wide text-white transition hover:-translate-y-0.5 hover:bg-app-caramelo-torrado focus:outline-none focus:ring-4 focus:ring-app-dourado-mel/25 sm:w-auto">
              Continuar
            </button>) : (<button type="button" onClick={criarRestaurante} disabled={isSubmitting} className="flex h-9 w-full items-center justify-center bg-app-dourado-mel px-6 text-xs font-bold uppercase tracking-wide text-white transition hover:-translate-y-0.5 hover:bg-app-caramelo-torrado focus:outline-none focus:ring-4 focus:ring-app-dourado-mel/25 disabled:cursor-not-allowed disabled:opacity-70 sm:w-auto">
        ...
        <div className="mt-2 flex flex-col gap-2 text-sm text-app-cinza sm:flex-row sm:items-center sm:justify-between">
          <span>
            Ja possui uma conta?{" "}
            <Link href="/login" className="font-bold text-app-cafe-profundo transition hover:text-app-caramelo-torrado">
              Entrar
            </Link>
          </span>
          {message ? (<span className="font-semibold text-app-caramelo-torrado">
              {message}
            </span>) : null}
        </div>
      </div>
