...
function IconeTema({ tema }) {
    ...
    return (<svg aria-hidden="true" viewBox="0 0 24 24" className="h-5 w-5">
    ...
    const opcoes = [
        {
            valor: "claro",
            titulo: "Modo claro",
            descricao: "Tons suaves e maior luminosidade",
        },
        {
            valor: "escuro",
            titulo: "Modo escuro",
            descricao: "Menos brilho e contraste confortável",
        },
    ];
    return (<fieldset className="mx-auto mt-7 w-full max-w-xl rounded-[8px] bg-app-chantilly p-5 shadow-sm ring-1 ring-app-baunilha-dourada/45">
      ...
      <div className="mt-1 grid gap-3 sm:grid-cols-2">
        {opcoes.map((opcao) => {
            ...
            return (<label key={opcao.valor} className={`flex cursor-pointer items-start gap-3 rounded-[8px] border p-4 transition ${selecionado
              ...
              <input type="radio" name="tema-aplicacao" value={opcao.valor} checked={selecionado} onChange={() => atualizarTema(opcao.valor)} className="mt-1 h-4 w-4 shrink-0 accent-app-caramelo-torrado"/>
