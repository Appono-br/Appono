...
const faqs = [
  ...
  },
  {
    ...
    answer:
      "Sim. O restaurante acompanha reservas, pedidos antecipados, capacidade e status operacional pelo módulo próprio.",
  },
];

export default function HomePage() {
  const [menuOpen, setMenuOpen] = useState(false);
  const [profileDialog, setProfileDialog] = useState<"cadastro" | "login" | null>(
    null,
  );
    ...
    <main className="min-h-screen bg-app-chantilly text-app-texto-escuro">
      <header className="sticky top-0 z-30 border-b border-app-baunilha-dourada/60 bg-app-chantilly/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-5">
          ...
          <div className="hidden items-center gap-3 sm:flex">
            <button
              ...
              className="border border-app-caramelo-torrado px-4 py-2 text-sm font-semibold text-app-caramelo-torrado transition hover:bg-app-creme-suave"
            >
              Criar conta
            </button>
            <button
          ...
          <button
            ...
            className="border border-app-caramelo-torrado px-3 py-2 text-sm font-semibold text-app-cafe-profundo md:hidden"
        ...
        {menuOpen ? (
          <div
            id="mobile-menu"
            className="border-t border-app-baunilha-dourada bg-app-creme-leve px-5 py-4 md:hidden"
          >
        ...
        <div className="mx-auto max-w-6xl px-5">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {valueCards.map((card) => (
              <article
                key={card.title}
                className="group bg-app-chantilly p-6 text-center shadow-sm transition duration-300 hover:-translate-y-1 hover:bg-app-cafe-profundo hover:text-app-creme-leve hover:shadow-md"
              >
      ...
      <section id="sobre" className="bg-app-creme-leve py-20">
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-5 md:grid-cols-2">
          ...
          <Image
            ...
            width={640}
            height={420}
            className="h-80 w-full object-cover shadow-sm transition duration-300 hover:brightness-95"
          />
        </div>
      </section>
            ...
            {faqs.map((faq, index) => (
              <div key={faq.question}>
                <button
                  ...
                  className="flex w-full items-center justify-between gap-4 py-5 text-left text-sm font-semibold text-app-cafe-profundo"
                >
                  {faq.question}
                ...
                {activeFaq === index ? (
                  <p className="pb-5 text-sm leading-6 text-app-mocha">
                    {faq.answer}
                  </p>
                ) : null}
              </div>
          ...
          <div className="w-full max-w-md bg-app-chantilly p-6 shadow-xl">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="border border-app-baunilha-dourada px-3 py-1 text-app-cafe-profundo"
                aria-label="Fechar"
              >
                X
              </button>
