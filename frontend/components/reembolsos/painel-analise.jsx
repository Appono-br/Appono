...
export function PainelAnaliseReembolsos({ perfil }) {
    ...
    async function analisar(item, decisao) {
        ...
        if (decisao === "RECUSAR") {
            ...
            if (!resposta) return;
        } else if (!window.confirm(`Aprovar o reembolso de teste do pedido #${item.id_pedido}? A operacao sera enviada ao sandbox do Mercado Pago, sem dinheiro real.`)) return;
    ...
    return <main className="min-h-screen bg-app-chantilly px-5 py-10 text-app-cafe-profundo"><section className="mx-auto max-w-6xl">
        ...
        <header className="mt-6 rounded-[16px] bg-app-cafe-profundo p-7 text-app-creme-leve"><p className="text-xs font-bold uppercase tracking-[0.18em] text-app-baunilha-dourada">Mercado Pago teste · Marketplace simulado</p><h1 className="mt-2 text-3xl font-semibold">Solicitacoes de reembolso</h1><p className="mt-3 max-w-3xl text-sm text-app-creme-suave">A aprovacao estorna o pagamento no ambiente de testes do Mercado Pago e ajusta o repasse simulado. Pagamentos reais sao bloqueados.</p></header>
