...
export function aplicarMascaraCodigoBanco(valor: string) {
  return somenteNumeros(valor).slice(0, 3);
}

export function aplicarMascaraAgencia(valor: string) {
  return somenteNumeros(valor).slice(0, 5);
}

export function aplicarMascaraConta(valor: string) {
