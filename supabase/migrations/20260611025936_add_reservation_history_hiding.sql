alter table public.reservas
  add column if not exists ocultada_cliente boolean not null default false,
  add column if not exists ocultada_restaurante boolean not null default false;
...
begin
  ...
  if cliente_id is not null then
    ...
    where reserva.id_reserva = reserva_id
      ...
      and reserva.status_reserva in ('CANCELADA', 'RECUSADA')
  ...
  if reserva_ocultada.id_reserva is null then
    raise exception 'Apenas reservas canceladas podem ser excluidas da lista';
  end if;
