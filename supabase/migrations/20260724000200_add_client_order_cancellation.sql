...
begin
  ...
  where pedido.id_pedido = pedido_id
    ...
    and reserva.id_reserva = pedido.id_reserva
    and pedido.status_pedido in ('PENDENTE', 'CONFIRMADO')
    and reserva.data_reserva + reserva.horario_inicio - interval '30 minutes' >= localtimestamp
  returning * into pedido_cancelado;

  if pedido_cancelado.id_pedido is null then
    raise exception 'Pedido nao encontrado ou nao pode mais ser cancelado. O cancelamento so e permitido ate 30 minutos antes da reserva';
  end if;
