...
create index if not exists restaurantes_favoritos_restaurante_idx on public.restaurantes_favoritos (id_restaurante);
create index if not exists avaliacoes_restaurante_restaurante_idx on public.avaliacoes_restaurante (id_restaurante, created_at desc);

drop trigger if exists set_updated_at on public.avaliacoes_restaurante;
create trigger set_updated_at before update on public.avaliacoes_restaurante
...
revoke all on table public.restaurantes_favoritos from anon, authenticated;
revoke all on table public.avaliacoes_restaurante from anon, authenticated;
...
create policy "Cliente gerencia seus restaurantes favoritos" on public.restaurantes_favoritos
...
create policy "Leitura publica de avaliacoes" on public.avaliacoes_restaurante
for select to anon, authenticated using (true);

drop policy if exists "Cliente gerencia suas avaliacoes" on public.avaliacoes_restaurante;
create policy "Cliente gerencia suas avaliacoes" on public.avaliacoes_restaurante
for all to authenticated
using (id_cliente in (select id_cliente from public.clientes where id_auth = (select auth.uid())))
with check (id_cliente in (select id_cliente from public.clientes where id_auth = (select auth.uid())));
